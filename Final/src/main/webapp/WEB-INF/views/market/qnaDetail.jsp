...
<div class="qna-detail-wrap">
   <form name="qnaupdateForm" role="form" method="post" action="/market/qnaUpdate">
	...
	<c:if test="${sessionScope.mk.marketerId eq qnadetail.pqnaReplywriter && qnadetail.prdQnastatus == 0 }">
		<div class="qna-reply-btn-wrap">
			<div class="reply-btn">
			    <button type="button" class="replyBtn" onclick="openAnswer()">판매자답변</button>
			    <button type="button" class="replyBtn" id="qnaList" onclick="listGo();">목록으로</button>
			</div>
		</div>
	</c:if>
</div>
							...
							<div>
								...
								<c:choose>
									<c:when
										...
										<p>
											<a href="/market/qnareplyDelete?prdQnano=${qnareply.prdQnano}"
