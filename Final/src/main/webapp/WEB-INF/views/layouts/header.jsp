			...
			<c:choose>
				<c:when test="${empty sessionScope.m && empty sessionScope.mk }">
					<ul class="navbar-nav ms-auto">
						...
						<li class="nav-item">
							<a class="nav-link" href="/sj.do">Register</a>
						</li>
					</ul>
					<!--일반 회원-->
				</c:when>
				<c:otherwise>
					<ul class="navbar-nav ms-auto">
						<c:if test="${sessionScope.m.userLevel eq 1}">
							<li class="nav-item">
								<a href="#" class="nav-link" id="font"> ${sessionScope.m.userName }</a>
							</li>
							<li class="nav-item">
								<a href="/logout.do" class="nav-link" id="font">logout</a>
							</li>
							<li class="nav-item">
								<a href="/mypage.do" class="nav-link" id="font"> 마이페이지</a>
							</li>
							<li class="nav-item">
						...
						<li class="nav-item">
							<c:if test="${sessionScope.m.userLevel eq 2}">
								<a href="#" id="font"  class="nav-link">[최고관리자] ${sessionScope.m.userId }</a>
							</c:if>
						</li>
						<!--판매자 회원  -->
